...
def asen():
    cursor = mydb.cursor()
    cursor.execute('SELECT * FROM senvalue')
    data = cursor.fetchall()
    return render_template('asen.html', result = data)
...
@app.route('/msen')
def msen():
    return render_template('msen.html')
...
@app.route('/ssend', methods = ['POST', 'GET'])
def ssend():
    if request.method == 'POST':
        mid = session['uid']
        ...
        v = (mid, sen, decMessage, key, encMessage)
        cursor = mydb.cursor()
        cursor.execute('insert into senvalue values(0, %s, %s, %s, %s, %s)', v)
        mydb.commit()
        if cursor.rowcount == 1:
            flash("Sensor Values Sent Successfuly...")
            return render_template('msen.html')
        else:
            flash("Sensor Values not Sent")
            return render_template('msen.html')
...
@app.route('/adata/<string:id>')
def adata(id):
    cursor = mydb.cursor()
    cursor.execute('SELECT * FROM senvalue where id='+id)
    data = cursor.fetchone()
    return render_template('adata.html', result = data)
